...
<header>
    ...
    $produits = $manager->getProduitsNormaux();
    echo '<div class="produits">';
    echo '<h2>Produits Normaux</h2>';
    ...
    echo '</div>';
    echo '</div>';
    $produits = $manager->getProduitsOffreSpeciale();
    echo '<div class="produits">';
    echo '<h2>Des produits en promotion</h2>';
    echo '<div class="produits-container">';
foreach ($produits as $produit) {
        echo '<fieldset class="produit">';
        echo '<img src="' . $produit->getImage() . '" alt="' . $produit->getNom() . '">';
        ...
        echo '<p>Couleur: ' . $produit->getColor() . '</p>';
        echo '<p>Motif: ' . $produit->getMotif() . '</p>';
        echo '<p>Was: <del>' . $produit->getPrix() . ' TND</del></p>';
        $nouveauPrix = $produit->getPrix() * (1 - Produits::REMISE);
        echo '<p>Now: ' . $nouveauPrix . ' TND</p>';
        $savePrix=$produit->getPrix()-$nouveauPrix;
        echo '<p>You Save:' . $savePrix . ' TND</p>';
        echo '<a href="ajouterPanier.php?id=' . $produit->getNom() . '">Ajouter au panier</a>';
        echo '</fieldset>';
    }
echo '</div>';
